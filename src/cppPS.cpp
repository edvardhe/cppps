...
Eigen::MatrixXd runWithRoi(Eigen::Matrix3d K_pixel, ROI roi) {
    ...
    std::string path = std::string(PROJECT_DIR) + "/color";
    ...
    std::vector<Eigen::Vector3d> light_positions = loadJsonToPositionMatrix(std::string(PROJECT_DIR) + "/light_positions.json", image_names);
...
int main(int argc, char** argv) {
    ...
    std::vector<ROI> regions_of_interest_test = {R_roi};
     ...
     for (const ROI& roi : regions_of_interest_test) {
         std::string example_dir = std::string(PROJECT_DIR) + "/Documents/ReportExamplesTest/" + roi.name;
    ...
    for (const ROI& roi : regions_of_interest) {
        ...
        Eigen::MatrixXd z = runWithRoi(K_pixel, roi);
        depth::saveDepthMap(z, std::string(PROJECT_DIR) + "/depthMapPatches",
